...
```bash
cd ai-sdlc-prototyping-workflow
nvm use
```
...
> [!WARNING]
> The `ai-sdlc-prototyping-workflow` is not a production ready application, it is a tool for prototyping. It is not
...
The refreshed GOV.UK brand is available and turned on by default in the `ai-sdlc-prototyping-workflow`. To turn it
...
```bash
docker build --target development --no-cache --tag ai-sdlc-prototyping-workflow:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 ai-sdlc-prototyping-workflow:development
```
...
```bash
docker build --no-cache --tag ai-sdlc-prototyping-workflow .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 ai-sdlc-prototyping-workflow
```
...
```bash
docker run -it --entrypoint /bin/ash ai-sdlc-prototyping-workflow
```
